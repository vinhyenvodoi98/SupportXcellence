...
const Deposit = ({ allowance, vaultAddress, tokenAddress, symbol, balance }: DepositInterface) => {
  ...
  const { data: depositHash , isLoading: isDepositLoading, isSuccess: isDepositSuccess, write: triggerDeposit } = useContractWrite({
    ...
    <>
      ...
      <dialog id="deposit" className="modal">
        ...
        <form method="dialog">
          ...
          <h3 className="font-bold text-lg">{isApproved ? "Deposit": "Approve"}</h3>
          <label className='label flex justify-between'>
            <span className='label-text'>Amount:   </span>
            <span className='label-text'>max {balance && formatEther(balance.toString())} {symbol}</span>
          </label>
          ...
          <div className='flex justify-end my-4'>
            <button className='btn btn-success' onClick={() => handle()}>{isApproved ? "Deposit" :  "Approve"}</button>
          </div>
        </div>
      </dialog>
    </>
