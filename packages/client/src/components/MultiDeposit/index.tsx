...
const MultiDeposit = ({
  ...
  // Get fee
  const { data: userSender, refetch: refetchUserFactory } = useContractReads({
    contracts: [
      {
  ...
  return (
    <div className='p-8 border-l grid grid-rows-5'>
      <h1>Multichain Vault</h1>
      <p>
        To be able to use the multichain feature you need to create a new
        account
      </p>
      <p>
        <strong>Account:</strong> {userAddress}
      </p>
        ...
        (userSender[1].result ? (
          <div className='grid grid-cols-2 gap-4 mt-8'>
            {asset &&
              ...
              asset[1].result != undefined && (
                <div
                  className={`${!isWithdraw && 'col-span-2'} grid grid-cols-1`}
                >
                  <MDeposit
                    userContractSender={
                      userContractAddresses[5]
                        .UserContractSender as `0x${string}`
                    }
          ...
          <button
            ...
            onClick={() =>
              triggerCreateUser({
                args: [
                  { owner: address },
                  userContractAddresses[420].UserFactory,
                ],
                value: userSender[0].result as any,
              })
            }
          >
            {`Create Account (fee: ${formatEther(
              BigInt(userSender[0].result.toString())
            )})`}
          </button>
        ))}
    </div>
